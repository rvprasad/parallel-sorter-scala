...
object Main extends App {
  ...
  options.addOption(CLIOption.builder("w").longOpt("numOfWorkers").hasArg.required(true)
    .desc("Number of workers to use").build)
  options.addOption(CLIOption.builder("i").longOpt("intsPerWorker").hasArg.required(true)
    .desc("Number of ints per workers").build)
  options.addOption(CLIOption.builder("f").longOpt("filename").hasArg.required(true)
    .desc("Name of file containing ints to sort").build)
  options.addOption(CLIOption.builder("t").longOpt("tempDir").hasArg.required(true).desc("Name of temp directory")
    .build)
  ...
  val cmdLine = try {
    ...
    case _: ParseException => {
      new HelpFormatter().printHelp("java -jar snmc-assembly.jar sorter.parallel.Main", options)
      sys.exit(-1)
    }
  }
